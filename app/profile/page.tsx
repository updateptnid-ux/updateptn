...
import {
  ...
  Loader2, BookOpen, Search, X, Eye, EyeOff, Save, KeyRound,
} from "lucide-react";
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (prodi && univ) {
        setProdiQuery(`${prodi} — ${univ}`);
        setSelectedProdi({ id: "saved", univ, prodi });
      } else if (univ) {
        setProdiQuery(univ);
      }
  ...
  useEffect(() => {
    function onOutside(e: MouseEvent) {
      if (prodiRef.current && !prodiRef.current.contains(e.target as Node)) setShowProdiDropdown(false);
    }
  ...
  // Autocomplete search
  useEffect(() => {
    if (selectedProdi) { setShowProdiDropdown(false); return; }
    if (prodiQuery.trim().length < 2) { setProdiSuggestions([]); setShowProdiDropdown(false); return; }
    const t = setTimeout(async () => {
      setProdiLoading(true);
      try {
        const { data, error } = await supabase.rpc("search_kampus_pintar", { keyword: prodiQuery.trim() });
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 py-2">
      ...
      <div className="flex items-center gap-4">
        <Avatar className="h-14 w-14 border-2 border-slate-200 rounded-2xl">
      ...
      <Card className="bg-white border border-slate-200 rounded-2xl p-5 md:p-6 shadow-xs space-y-5">
        ...
        <form onSubmit={handleProfileSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5" ref={prodiRef}>
            <Label className="text-xs font-bold text-slate-700 flex items-center gap-1">
              <Target className="h-3 w-3 text-blue-600" /> Target Prodi &amp; Universitas Impian
            </Label>
            <div className="relative">
              <div className="relative bg-white rounded-xl border border-slate-200 flex items-center focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20 transition-all">
                {selectedProdi ? <BookOpen className="absolute left-3.5 h-4 w-4 text-blue-600" /> : <Search className="absolute left-3.5 h-4 w-4 text-slate-400" />}
                <input
                  type="text" value={prodiQuery}
                  onChange={e => { setProdiQuery(e.target.value); if (selectedProdi) { setSelectedProdi(null); setTargetUnivValue(""); setTargetProdiValue(""); } }}
                  onFocus={() => { if (prodiSuggestions.length > 0 && !selectedProdi) setShowProdiDropdown(true); }}
                ...
                />
                {prodiLoading && <Loader2 className="absolute right-3.5 h-4 w-4 animate-spin text-blue-600" />}
                {prodiQuery && !prodiLoading && (
                  <button type="button" onClick={handleClearProdi} className="absolute right-3.5 text-slate-400 hover:text-slate-600 p-1">
              ...
              {showProdiDropdown && !selectedProdi && (
                <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-slate-200 rounded-xl shadow-xl z-50 max-h-52 overflow-y-auto divide-y divide-slate-100 animate-in fade-in zoom-in-95">
                  {prodiSuggestions.map(item => {
                    ...
                    return (
                      <button key={item.id} type="button" onClick={() => handleSelectProdi(item)}
                        ...
                        <Avatar className="h-8 w-8 rounded-lg border border-slate-100 shrink-0">
                          ...
                          <AvatarFallback className="bg-blue-600 text-white font-bold text-[10px]">{getInitials(item.univ)}</AvatarFallback>
                        </Avatar>
                        <div className="overflow-hidden">
                          <p className="text-xs font-extrabold text-slate-900 group-hover:text-blue-600 truncate">{item.prodi}</p>
                          <p className="text-[11px] text-slate-500 font-medium truncate">{item.univ}</p>
                        </div>
                        <Badge variant="outline" className="ml-auto shrink-0 text-[10px] font-bold px-1.5 py-0">
                          {item.jenjang || "S1"}
                        </Badge>
                      </button>
                    );
            ...
            {selectedProdi && (
              <p className="text-[11px] text-blue-600 font-semibold pl-1">
                ✓ {selectedProdi.prodi} — {selectedProdi.univ}
              </p>
            )}
