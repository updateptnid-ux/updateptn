...
import { Clock, FileText, PlayCircle, Lock, Hourglass, Sparkles } from "lucide-react";
...
export default function StudentTryoutList({
  ...
  const [subStatus, setSubStatus] = useState<string | null>(initialSubscription?.status || null);
          ...
          return (
            <StaggerItem key={to.id}>
              <MotionCard className="h-full rounded-2xl">
                <Card className="bg-white/90 backdrop-blur-md border border-slate-200/80 shadow-xs rounded-2xl p-6 flex flex-col justify-between h-full hover:shadow-md transition-shadow">
